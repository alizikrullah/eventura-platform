...
export const getAllEvents = async (filters: EventFilters) => {
  ...
  if (date_from || date_to) {
    where.start_date = {};
    if (date_from)
      where.start_date.gte = new Date(date_from + "T00:00:00+07:00");
...
export const createEvent = async (
  ...
  const ticketTypesArray: any[] =
    typeof ticket_types === "string"
      ? JSON.parse(ticket_types)
      : ticket_types || [];

  // Auto-calculate total_seats dari sum kuantitas semua ticket types
  const computedTotalSeats =
    ticketTypesArray.length > 0
      ? ticketTypesArray.reduce(
          (sum: number, tt: any) => sum + Number(tt.available_quantity),
          0,
        )
...
export const updateEvent = async (
  ...
  const ticketTypesArray: any[] | undefined = ticketTypesRaw
    ? typeof ticketTypesRaw === "string"
      ? JSON.parse(ticketTypesRaw)
      : ticketTypesRaw
    : undefined;
  ...
  if (ticketTypesArray && ticketTypesArray.length > 0) {
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (tt.db_id) {
          ...
          if (existingTt) {
            ...
            const soldQty = Math.max(
              0,
              existingTt.quantity - existingTt.available_quantity,
            );
      ...
      const newAvailableSeats = allTickets.reduce(
